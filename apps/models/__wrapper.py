...
class Wrapper(object):
	...
	def select(self, columns, table_names, condition = ""):
		...
		try:
			...
			print "select {0} from {1} {2}".format( escape_string(columns), escape_string(table_names), escape_string(condition) )
			cursor.execute("select %s from %s %s" % ( escape_string(columns), escape_string(table_names), escape_string(condition) ))
